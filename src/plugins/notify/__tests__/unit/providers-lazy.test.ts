...
vi.mock("../../providers/tauri", () => {
  throw new Error("notify providers/tauri was evaluated on the web path");
});
...
const createWebCtx = (): NotifyContext => ({
  config: {},
...
describe("loadNotifyProvider — lazy Tauri boundary", () => {
  ...
  it("resolves the web provider without evaluating the tauri module", async () => {
    const provider = await loadNotifyProvider(createWebCtx())();
